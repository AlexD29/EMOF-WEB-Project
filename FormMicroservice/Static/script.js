...
function updateQuestionPage() {
	...
	document.getElementById("question").textContent = Object.keys(formInfo.questions)[pageCounter-1] + ". " + Object.values(formInfo.questions)[pageCounter-1] 
...
function createUserInputQuestion(question) {
	...
	if(selectElement.id == "Age"){
		...
		const emptyOption = document.createElement("option");
		emptyOption.value = "";
		emptyOption.textContent = "Select the platform from where you found the form";
		selectElement.appendChild(emptyOption);
...
function getUserInfoResponses() {
    const inputs = document.getElementById('user-info-container').getElementsByTagName('select');
